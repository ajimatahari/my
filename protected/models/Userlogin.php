...
class Userlogin extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('username, password, hape, email', 'required'),
			array('email', 'numerical', 'integerOnly'=>true),
			array('username', 'length', 'max'=>25),
			array('hape', 'length', 'max'=>15),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, username, password, hape, email', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('hape',$this->hape,true);
